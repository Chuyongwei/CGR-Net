...
'''
@Title: icml_glo_lcl21
@Author: ChuyongWei
@Date: 2025-10-12
@Version: 1.0
@Description:
加入ACMatch的lclayer和MGSA的glo块
因为残差是有效果的所以我们可以考虑加入残差的效果
MGSA的glo块替换gcn
...
class KNNFeats(nn.Module):
    def __init__(self, in_dim, out_dim, use_bn=True):
        ...
        if use_bn:
            # TODO 这里输入是cat_filter的结果，应该修改输入为out_dim
    ...
    def forward(self, feats):
        # BCN1
        nn_feats = get_graph_feature(feats)  # B 2C N K

        nn_feats = self.cat_filter(nn_feats)  # BCNK
        # 增强特征
        nn_feats_out = self.mlp(nn_feats)  # b*c*n*k
        # 它沿着第 3 个维度（即 K 个邻居的维度）进行最大值操作。
...
class DS_Block(nn.Module):
    def __init__(self, initial=False, predict=False, out_channel=128, k_num=8, sampling_rate=0.5):
        ...
        self.embed_0 = nn.Sequential(
            ...
            LCLayer(self.out_channel, self.out_channel),
            ResNet_Block(self.out_channel, self.out_channel, pre=False),
            ResNet_Block(self.out_channel, self.out_channel, pre=False),
            # CGCE
