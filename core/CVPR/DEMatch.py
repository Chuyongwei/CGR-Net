...
class AttentionPropagation(nn.Module):
    ...
    def forward(self, x1, x2, kv_mask=None):
        ...
        if self.mode == 'full':
            ...
            add_value = torch.einsum("bhdn,bhdv,bhn->bhvn", Q, KV, Z).reshape(batch_size, self.head_dim * self.head,
                                                                              -1) * v_length  # B(HD)N
        else:
            raise KeyError
...
class InlinerPredictor(nn.Module):
    def __init__(self, channels):
        ...
        self.inlier_pre = nn.Sequential(
            nn.Conv1d(channels, 64, kernel_size=1), nn.InstanceNorm1d(64, eps=1e-3), nn.BatchNorm1d(64),
            nn.ReLU(inplace=True),
            nn.Conv1d(64, 16, kernel_size=1), nn.InstanceNorm1d(16, eps=1e-3), nn.BatchNorm1d(16),
            nn.ReLU(inplace=True),
...
class DeMatch(nn.Module):
    def __init__(self, config):
        ...
        self.layer_blocks = nn.Sequential(
            *[LayerBlock(config.net_channels, config.head, config.layer_names, mode=config.attention_mode) for _ in
